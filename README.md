...
这是论文《基于深度学习的 X 光片手骨骨龄预测》的配套工程仓库，围绕 RHPE 数据集实现手部 X 光片骨龄回归。当前主工程位于 `src/rhpe_boneage/`，通过 `scripts/*.py` 提供训练、验证、测试、推理、数据检查、Optuna 搜索和 Tk UI。仓库同时保留了 `SIMBA/` 与 `Bonet-master/` 两份参考实现，但当前主流程不会直接 import 或执行这些旧代码。

论文应用语境包括骨骼发育评估、法医学年龄辅助判断和公安数字影像智能分析方法探索。模型输出只能作为年龄相关分析的辅助线索，不能直接替代法医学综合鉴定意见。
...
- 论文主实验以 RHPE 验证集作为本地可计算的最终评估依据

这意味着：

- `experiment.mode` 主要用于实验声明和一致性告警，不会偷偷切换模型。
- 实际行为由 `model.*`、`data.*`、`training.*` 配置决定。
- 当前项目是“继承 SIMBA 和 BoNet 思想的工程实现”，不是两篇论文原始代码的逐行复现。

## 与论文的对应关系

本仓库对应论文文件 `基于深度学习的X光片手骨骨龄预测.docx`。论文第 3 章中的方法名称可概括为：

```text
基于身份先验与全局—局部热力引导的多模态手骨骨龄预测方法
```
...
1. 输入解析与预处理：读取 X 光灰度图、ROI JSON 和 CSV 元信息，解析 bbox/keypoints，生成全局裁剪图、全局 heatmap、局部 patch、局部 heatmap 和 ROI 几何向量。
2. 多分支特征编码：全局分支、局部分支、元信息模块和 ROI 几何模块分别编码不同来源的信息。
3. 多模态融合：融合全局特征、局部区域特征、元信息特征和 ROI 几何特征，形成统一表示。
4. 骨龄回归与恢复：先预测相对骨龄 `Boneage - Chronological`，再加回 `Chronological` 得到最终骨龄。
...
该目录的 `best_metrics.json` 显示：best epoch 为 74，验证集 `best_val_mae=7.4158`、`best_val_mad=5.8906`、`val_r2=0.9446`。论文正文和对比表中报告的 `MAD=7.42` 对应仓库里的平均绝对误差 `best_val_mae`；仓库字段 `best_val_mad=5.8906` 是中位绝对误差。由于 RHPE test CSV 不含 `Boneage`，论文和本仓库都以验证集作为本地可计算指标的最终评估依据。
...
下面这些结果来自当前仓库里已经存在的本地输出目录，适合用来对照 README 与产物格式。指标来自各目录的 `best_metrics.json`，单位与 RHPE 骨龄标签一致。

表中的 `Val MAE` 对应论文表 6、表 7 中用于对比的 `MAD` 口径；表中的 `Val median AE` 对应仓库字段 `best_val_mad`，是中位绝对误差。

| 输出目录 | 说明 | best epoch | Val MAE / 论文 MAD | Val median AE | Val R2 |
| --- | --- | ---: | ---: | ---: | ---: |
...
论文中的主结果对应 `150-8-no2` 这一行，即 `Val MAE=7.42`、`Val R2=0.9446`。论文中与 SIMBA 的相同条件比较可对应 `90-8-no1-SIMBA_like_train_20260410_190034`，即 `SIMBA=9.74`、`Ours=7.42`，下降 2.32，约 23.8%。

论文消融表可按 `Val MAE / 论文 MAD` 读取：

| Methods | EfficientNet-B0 | Heatmap Guidance | CBAM | Local Branch | MAD |
| --- | --- | --- | --- | --- | ---: |
| w/o EfficientNet-B0 | 否 | 是 | 是 | 是 | 10.46 |
| w/o Heatmap Guidance | 是 | 否 | 是 | 是 | 7.77 |
| w/o CBAM | 是 | 是 | 否 | 是 | 9.80 |
| w/o Local Branch | 是 | 是 | 是 | 否 | 7.58 |
| Ours | 是 | 是 | 是 | 是 | 7.42 |
...
- 论文实验仅基于 RHPE 数据集完成，尚未在更大规模或不同来源数据集上验证泛化能力。
- 从论文可视化分析看，模型在高骨龄区间和少量复杂样本上仍存在更大的离散误差。
...
- 指标：论文表中 `MAD=7.42` 对应仓库 `best_val_mae=7.4158`，仓库 `best_val_mad=5.8906` 是中位绝对误差
